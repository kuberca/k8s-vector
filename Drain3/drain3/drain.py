...
semantics = ["delete", "get", "update", "add", "create", "bind", "deleting", "updating", "adding", "binding", "deletion", "bound", "available", "ready", "pending", "released", "true", "false", "failed", "error", "success", "successful", "warning", "succeeded"]
    ...
    def add_log_message(self, content: str):
        ...
        # Add the new log message to the existing cluster
            ...
            if ' '.join(new_template_tokens) != ' '.join(match_cluster.log_template_tokens):
                # if different token falls within semantics, then need to creat a new cluster
                create_new_cluster = False

                for i in range(len(content_tokens)):
                    if content_tokens[i] != match_cluster.log_template_tokens[i]:
                        if content_tokens[i].lower() in semantics:
                ...
                if create_new_cluster:
                    self.clusters_counter += 1
                    cluster_id = self.clusters_counter
                    new_match_cluster = LogCluster(content_tokens, cluster_id)
                    self.id_to_cluster[cluster_id] = new_match_cluster
                    self.add_seq_to_prefix_tree(self.root_node, new_match_cluster)
                    update_type = "cluster_created"
                else:
                    print("content: ", content)
                    print("old template: ", ' '.join(match_cluster.log_template_tokens))
                    print("new template: ", ' '.join(new_template_tokens), "\n")
